...
def traverse_node(node, collected_tokens, byte_code, include_comments=False):
    ...
    comment_types = [
        ...
        'line_comment',
        'block_comment',
        'jsx_comment',       # JavaScript / JSX
        'html_comment',      # JavaScript / HTML embeds
    ]
    ...
    for child in node.children:
        if child.child_count == 0:
            ...
            if (
                token not in exclude_token
                and token.strip() != ''
                and (include_comments or child.type not in comment_types)
            ):
                token_info = {}
                ...
                # CodeSearchNet's Java tokenizer removes the contents of a
                # character literal and keeps its opening/closing quotes as
                # two tokens (for example, '.' becomes "'", "'").
                if token in {'/>', '</'}:
                    first_part = copy.deepcopy(token_info)
                    first_part['token'] = token[0]
                    first_part['end_byte'] = child.start_byte + 1

                    second_part = copy.deepcopy(token_info)
                    second_part['id'] = (child.id, 'operator_part')
                    second_part['token'] = token[1]
                    second_part['start_byte'] = child.start_byte + 1

                    collected_tokens.extend([first_part, second_part])
                elif (
                    child.type == 'hex_integer_literal'
                    and len(token) > 1
                    and token[-1] in {'l', 'L'}
                ):
                    number = copy.deepcopy(token_info)
                    number['token'] = token[:-1]
                    number['end_byte'] = child.end_byte - 1

                    suffix = copy.deepcopy(token_info)
                    suffix['id'] = (child.id, 'integer_suffix')
                    suffix['token'] = token[-1]
                    suffix['start_byte'] = child.end_byte - 1

                    collected_tokens.extend([number, suffix])
                elif (
                    child.type == 'character_literal'
                    and len(token) >= 2
                    and token[0] == token[-1] == "'"
                ):
                    opening_quote = copy.deepcopy(token_info)
                    opening_quote['token'] = "'"
                    opening_quote['end_byte'] = child.start_byte + 1

                    closing_quote = copy.deepcopy(token_info)
                    closing_quote['id'] = (child.id, 'closing_quote')
                    closing_quote['token'] = "'"
                    closing_quote['start_byte'] = child.end_byte - 1

                    collected_tokens.extend([opening_quote, closing_quote])
                else:
